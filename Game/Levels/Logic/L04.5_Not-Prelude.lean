...
## How to prove `False`?
Well, when was the first you saw `False`?
Here:
'
Negation of a proposition `P`, denoted `¬P`, is defined as `P → False`. 
'
It should be clear that to get to false, you would need to prove `¬P`, `P`. Then given such a proof state:
```
hP : P
hnP : ¬P
```
...
Moreover, `False` has no introduction rule. If it did, or if you were able to find `h:False` then our system is worthless because we can prove anything. Such a system would be called an inconsistent system because of a contradiction.
